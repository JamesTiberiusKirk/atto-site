...
function GenerateEmailString({
  ...
  return ReactDOMServer.renderToString(
    <>
      ...
      <p style={{ fontSize: "15px" }}>In the past 24 hours we have:</p>
      <p style={{ fontSize: "15px" }}>
        {applications && aLen === 0
          ? "No new applications"
          : `Applications: ${aLen} new`}
      </p>

      <p style={{ fontSize: "15px" }}>
        {contactRequests && cLen === 0
          ? "No new contact us requests"
          : `Contact us requests: ${cLen} new`}
      </p>

      <p style={{ fontSize: "15px" }}>
        {newsSubscriptions && nLen === 0
          ? "No new news letter subscriptions"
          : `News letter subscriptions: ${nLen} new`}
      </p>
      ...
      {contactRequests && contactRequests?.length > 0 && (
        ...
        >
          <p style={{ fontSize: "20px" }}>Contact requests:</p>
          ...
          {contactRequests.map((c, i) => (
            <div
              ...
              key={i}
            >
              <p style={{ fontSize: "15px" }}>Name: {c.name}</p>

              <p style={{ fontSize: "15px" }}>Email: {c.email}</p>

              <p style={{ fontSize: "15px" }}>Pronouns: {c.pronouns}</p>

              <p style={{ fontSize: "15px" }}>Message: {c.message}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {newsSubscriptions && newsSubscriptions?.length > 0 && (
        ...
        >
          <p style={{ fontSize: "20px" }}>New news subscriptions:</p>
...
export default async function AdminReportingHandler(
  req: NextApiRequest,
  res: NextApiResponse
) {
  ...
  if (
    applications?.error ||
    contactRequests?.error ||
    newsSubscriptions?.error
  ) {
    console.error(
      "Error:",
      applications?.error,
  ...
  if (
    ...
    newsSubscriptions?.data?.length === 0
  ) {
